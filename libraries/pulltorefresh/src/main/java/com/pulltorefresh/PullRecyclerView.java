...
public class PullRecyclerView extends RecyclerView {
    private final static String TAG = "PullRecyclerView";
        ...
        public void onChanged() {
            ...
            if(adapter instanceof HeaderAndFooterRecyclerViewAdapter){
                ...
                if(headerAndFooterAdapter.getInnerAdapter() != null && mEmptyView != null) {
                    ...
                    if(count == 0) {
                        mEmptyView.setVisibility(View.VISIBLE);
                        PullRecyclerView.this.setVisibility(View.GONE);
                    } else {
                        mEmptyView.setVisibility(View.GONE);
                        PullRecyclerView.this.setVisibility(View.VISIBLE);
                    }
                }
            }else {
                if(adapter != null && mEmptyView != null) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                mLastY = ev.getRawY();
                Log.i(TAG,"mLastY---ACTION_DOWN------>"+mLastY);
                break;
            case MotionEvent.ACTION_MOVE:
                final float deltaY = ev.getRawY() - mLastY;
                Log.i(TAG,"deltaY---ACTION_MOVE------>"+deltaY);
                mLastY = ev.getRawY();
                Log.i(TAG,"setVisibleHeight----"+mRefreshHeader.getVisibleHeight());
//                mRefreshHeader.setVisibleHeight(mRefreshHeader.getVisibleHeight());
                Log.i(TAG,"mLastY---ACTION_MOVE------>"+mLastY);
                if (isOnTop() && pullRefreshEnabled) {
                    Log.i(TAG,"deltaY---ACTION_MOVE---onMove------>"+deltaY/DRAG_RATE);
                    mRefreshHeader.onMove(deltaY / DRAG_RATE);
                    Log.i(TAG,"mRefreshHeader.getVisibleHeight()---"+mRefreshHeader.getVisibleHeight()+"--mRefreshHeader.getState()--"+mRefreshHeader.getState());
